...
{
    public class MssqlElasticPoolResourceStateHelper
    {
        ...
        /// <summary>
        /// Find the closes DTU (or the one provided) that can hold the required storage
        /// </summary>
        ...
        public static async Task<Dictionary<string, string>> ExpandElasticPoolWildcard(ArmClient client, string key, string resourceId, ILogger logger, CancellationToken stoppingToken)
        {
            ...
            // Get all elastic pools in the server
            var elasticPools = server.Value.GetElasticPools();

            // Build the list of resource IDs for each elastic pool
