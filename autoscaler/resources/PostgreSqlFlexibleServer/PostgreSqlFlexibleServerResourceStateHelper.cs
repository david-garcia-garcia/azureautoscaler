...
using poolautoscaler.resources.PostgreSqlFlexibleServer.Dto;
using poolautoscaler.utils;

namespace poolautoscaler.resources.PostgreSqlFlexibleServer
{
    /// <summary>
    /// Helper for PostgreSQL Flexible Server SKU comparison and capacity values.
    /// </summary>
    public static class PostgreSqlFlexibleServerResourceStateHelper
    {
        /// <summary>All supported PostgreSQL Flexible Server SKUs with tier, name and IOPS bounds.</summary>
        ...
        [
            ...
            new SkuInfo("MemoryOptimized", "Standard_E2ds_v5", 360, 5000),
            new SkuInfo("MemoryOptimized", "Standard_E4ds_v5", 360, 10000),
            new SkuInfo("MemoryOptimized", "Standard_E8ds_v5", 360, 18000),
            new SkuInfo("MemoryOptimized", "Standard_E16ds_v5", 360, 28800),
            new SkuInfo("MemoryOptimized", "Standard_E20ds_v5", 360, 28800),
            new SkuInfo("MemoryOptimized", "Standard_E32ds_v5", 360, 38000),
            new SkuInfo("MemoryOptimized", "Standard_E48ds_v5", 360, 48000),
            new SkuInfo("MemoryOptimized", "Standard_E64ds_v5", 360, 64000),
            new SkuInfo("MemoryOptimized", "Standard_E96ds_v5", 360, 80000)
        ];

        /// <summary>Gets the list of valid SKU names for the server's tier.</summary>
        /// <param name="resource">The PostgreSQL flexible server ARM resource.</param>
        /// <returns>Array of SKU names.</returns>
        public static string[] GetCapacityValues(ArmResource resource)
        {
            if (!(resource is PostgreSqlFlexibleServerResource server))
            {
        ...
        /// <param name="tier">The tier (Burstable, GeneralPurpose, MemoryOptimized).</param>
        /// <returns>Array of SKU names.</returns>
        public static string[] GetCapacityValues(string tier)
        {
            switch (tier)
            {
                ...
                case "MemoryOptimized": return AllSkus.Where(s => s.Tier == "MemoryOptimized").Select(s => s.Sku).ToArray();
                case "GeneralPurpose": return AllSkus.Where(s => s.Tier == "GeneralPurpose").Select(s => s.Sku).ToArray();
                default: throw new ArgumentException("Invalid SKU tier: " + tier);
            }
        }
        ...
        public static string GetMinimumSkuThatSatisfiesCoreCount(int coreCount, string tier, int millicoresPerHour = 0)
        {
            ...
            foreach (var capacity in capacities)
            {
                ...
                if (tier == "Burstable" && millicoresPerHour > 0)
                {
            ...
            throw new Exception($"Could not find a suitable SKU to accommodate {coreCount} cores and {millicoresPerHour} burstable_credits/core");
        }
