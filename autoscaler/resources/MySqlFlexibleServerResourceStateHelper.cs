...
namespace poolautoscaler.resources
{
    ...
    public static class MySqlFlexibleServerResourceStateHelper
    {
        ...
        public static string[] GetCapacityValues(string tier)
        {
            ...
            {
                case "Burstable":
                    return AllSkus.Where(s => s.Tier == "Burstable").Select(s => s.Sku).ToArray();
                case "BusinessCritical":
                    return AllSkus.Where(s => s.Tier == "BusinessCritical").Select(s => s.Sku).ToArray();
                case "GeneralPurpose":
                    return AllSkus.Where(s => s.Tier == "GeneralPurpose").Select(s => s.Sku).ToArray();
                default:
                    throw new ArgumentException("Invalid SKU: " + tier);
            }
        }

        public static int GetCoreCountFromSkuName(string sku)
        {
            var count = int.Parse(Regex.Match(sku, @"[A-Z](\d+)").Groups[1].Value);
            return count;
        }
        ...
        public static string GetMinimumSkuThatSatisfiesCoreCount(int coreCount, string tier, int millicoresPerHour = 0)
        {
            ...
            foreach (var capacity in capacities)
            {
                ...
                // Make sure the SKU is able to absorb the millicore usage without depleting excesively the amount of credits
                var forecastedCpuAverageUsage = (millicoresPerHour / (cores * 10));
                if (forecastedCpuAverageUsage > 0)
                {
                    // The max credits you can burn per hour = number of vCPUs × (100% - baseline performance) × 60 minute
        ...
        public static SkuInfo GetMinimumSkuThatSatisfiesIops(int iops, string? tier = null)
        {
            var skus = tier == null 
                ? AllSkus 
                : AllSkus.Where(s => s.Tier == tier).ToArray();

            var suitableSkus = skus.Where(s => s.MaxIops >= iops).ToArray();
            
            if (!suitableSkus.Any())
            {
                throw new Exception($"Could not find a suitable SKU to accommodate {iops} IOPS" + 
                    (tier != null ? $" for tier {tier}" : ""));
            }

            // Return the SKU with the lowest MaxIops that still satisfies the requirement
        ...
        public static int CompareSku(string tier, string dimensionValue1, string dimensionValue2)
        {
            ...
            var capacityValues = MySqlFlexibleServerResourceStateHelper.GetCapacityValues(tier);
