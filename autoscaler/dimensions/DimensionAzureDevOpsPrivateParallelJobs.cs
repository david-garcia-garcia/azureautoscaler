...
{
    /// <summary>
    /// Dimension for scaling Azure DevOps self-hosted (private) parallel jobs.
    ...
    internal class DimensionAzureDevOpsPrivateParallelJobs : IDimension
    {
        ...
        public void ValidateRuleConfiguration(ScalingRule rule)
        {
            ...
            if (!string.IsNullOrEmpty(rule.DimensionValueMin))
            {
                ...
                {
                    throw new ArgumentException($"DimensionValueMin must be a non-negative integer for PrivateParallelJobs dimension. Got: {rule.DimensionValueMin}");
                }
            }
            ...
            if (!string.IsNullOrEmpty(rule.DimensionValueMax))
            {
                ...
                {
                    throw new ArgumentException($"DimensionValueMax must be a non-negative integer for PrivateParallelJobs dimension. Got: {rule.DimensionValueMax}");
                }
            }
        }
            ...
            string value)
        {
            ...
            {
                throw new ArgumentException($"Invalid value for PrivateParallelJobs: {value}. Must be a non-negative integer.");
            }

            state.SetPrivateParallelJobs(count);
