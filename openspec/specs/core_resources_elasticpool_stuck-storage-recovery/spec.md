# Elastic pool stuck storage recovery
...
- **THEN** the instance's `storageRecoveryBumpBytes` is incremented by 50 GB
...
- **THEN** the computed `patch.MaxSizeBytes` is increased by `storageRecoveryBumpBytes` before tier-snapping
...
- **THEN** `storageRecoveryBumpBytes` resets to zero and the recovery cycle restarts from the formula-computed target
